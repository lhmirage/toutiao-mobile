...
import { getCurrentUser } from '@/api/user'// 加载请求方法
export default {
  data () {
    return {
      currentUser: {}// 当前登录用户信息,不能和user同名
    }
  },
  computed: {
    ...mapState(['user'])
  ...
  methods: {
    async loadCurrentUser () {
      const { data } = await getCurrentUser()// 发请求拿数据,把数据放到data里面,最终对着试图模板一个一个绑定接口返回的数据
